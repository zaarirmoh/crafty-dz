...
export default function StatCard({ label, value, icon: Icon, hint, className }: StatCardProps) {
  return (
    <div className={cn('group hover-raise rounded-xl border bg-card p-5', className)}>
      <div className="flex items-center justify-between gap-2">
        <p className="eyebrow">{label}</p>
        {Icon && (
          <Icon className="size-5 text-muted-foreground transition-colors group-hover:text-copper" />
        )}
      </div>
